...
IG_RESERVED = {
    ...
    "advertising", "testimonial", "title", "tag", "tags", "location", "locations",
    "guides", "shop", "checkout", "careers", "jobs", "business", "policies",
    "safety", "security", "cookies", "contact", "support", "media", "audio",
    "music", "effects", "ar", "filter", "filters", "hashtag"
}
...
def extract_contacts(base_url):
    ...
    for path in SUBPAGES_TO_CHECK:
        ...
        # Do not attempt JS rendering if HTTP request failed completely (e.g. 404 or connection error)
        if soup is not None:
        ...
        # 1. If email, phone, and instagram are all found, no need to check further subpages!
        if emails and phones and instagram:
            break

        # 2. If on homepage and we already have Instagram + contact info (email or phone), skip extra subpages!
        if path == "" and instagram and (emails or phones):
            break
